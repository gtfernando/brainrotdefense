...
local BuildingsData = require(dataFolder:WaitForChild("Buildings"))
...
local ZONE_FILL_COLOR = Color3.fromRGB(255, 0, 0)
local ZONE_FILL_TRANSPARENCY = 0
local ZONE_OUTLINE_COLOR = Color3.fromRGB(175, 13, 13)
local ZONE_OUTLINE_TRANSPARENCY = 0
local PLACEMENT_FILL_COLOR = Color3.fromRGB(255, 0, 0)
local PLACEMENT_FILL_TRANSPARENCY = 0.35
local PLACEMENT_OUTLINE_COLOR = Color3.fromRGB(147, 3, 3)
...
local RING_ROTATION = CFrame.Angles(0, math.rad(90), math.rad(90))
...
function HoverMouse.new(tool: Tool)
	...
	self._hoveredPlacementModel = nil :: Instance?
	self._hoveredPlacementAsset = nil :: string?
	self._placementPickupPacket = pickupPacket
	self._pendingPlacementPickups = {} :: { [string]: boolean }
...
function HoverMouse:HandleActivation()
	...
	if self._hoverMode == "placement" and self._hoveredPlacementId then
		self:_attemptPlacementPickup(self._hoveredPlacementId)
		return
	end

	if self._hoverMode == "zone" and self._hoveredZoneId then
...
local function findModelBasePart(model: Model): BasePart?
	...
	local descendant = model:FindFirstChildWhichIsA("BasePart", true)
	if descendant then
		return descendant
	end
...
function HoverMouse:_stopRingTween()
...
function HoverMouse:_ensureSelectionRing(): Part
...
function HoverMouse:_updateSelectionRing(model: Model)
...
function HoverMouse:_showSelectionRing(model: Model, placementId: string)
...
function HoverMouse:_hideSelectionRing()
	local ring = self._selectionRing
	if not ring then
		return
	end
...
function HoverMouse:_destroySelectionRing()
	self:_stopRingTween()
...
function HoverMouse:_setHighlightStyle(fillColor: Color3, fillTransparency: number, outlineColor: Color3, outlineTransparency: number)
...
function HoverMouse:_applyZoneHighlightStyle()
	self:_setHighlightStyle(ZONE_FILL_COLOR, ZONE_FILL_TRANSPARENCY, ZONE_OUTLINE_COLOR, ZONE_OUTLINE_TRANSPARENCY)
end

function HoverMouse:_applyPlacementHighlightStyle()
	self:_setHighlightStyle(PLACEMENT_FILL_COLOR, PLACEMENT_FILL_TRANSPARENCY, PLACEMENT_OUTLINE_COLOR, PLACEMENT_OUTLINE_TRANSPARENCY)
end

function HoverMouse:_resolvePlacementHover(result: RaycastResult?)
	if not result then
		return nil, nil, nil
	end
	...
	local target: Instance? = if instance:IsA("Model") then instance else instance:FindFirstAncestorOfClass("Model")
	if not target then
		return nil, nil, nil
	end
	...
	if typeof(ownerId) == "number" and ownerId ~= self._player.UserId then
		return nil, nil, nil
	end

	local placements = Controller:GetPlacements()
	local placementEntry = placements and placements[placementId]
	if not placementEntry then
		return nil, nil, nil
	end

	local assetId = placementEntry.asset or target:GetAttribute("PlacementAsset")

	return placementId, target, assetId
end
...
function HoverMouse:_updateHoverFromMouse()
	...
	local cell, hitResult = self:_getMouseCell(plot)
	if not cell then
		self:_clearHover()
		return
	end

	local placementId, placementModel, placementAsset = self:_resolvePlacementHover(hitResult)
	if placementId and placementModel then
		self:_setHoverPlacement(placementId, placementModel, placementAsset)
		return
	end
...
function HoverMouse:_setHoverZone(zone)
	...
	self._hoveredPlacementAsset = nil

	self:_applyZoneHighlight(zone)
	self:_hideSelectionRing()
end

function HoverMouse:_setHoverPlacement(placementId: string, target: Instance, assetId: string?)
	if typeof(placementId) ~= "string" or not target then
		self:_clearHover()
		return
	end
	...
	self._hoverMode = "placement"
	self._hoveredPlacementId = placementId
	self._hoveredPlacementModel = target
	self._hoveredPlacementAsset = assetId
	self._hoveredZone = nil
	self._hoveredZoneId = nil

	self:_applyPlacementHighlight(target)

	local isBuilding = assetId and BuildingsData[assetId] ~= nil
	if isBuilding and target:IsA("Model") then
		if not previousId or previousId ~= placementId or not self._selectionRing then
			self:_showSelectionRing(target, placementId)
		else
			self:_updateSelectionRing(target)
		end
	else
		self:_hideSelectionRing()
	end
...
function HoverMouse:_applyPlacementHighlight(target: Instance)
	self:_applyPlacementHighlightStyle()
...
function HoverMouse:_clearZoneUi()
	if self._activeZoneUi then
		self._activeZoneUi:Destroy()
		self._activeZoneUi = nil
		self._activeZoneUiZoneId = nil
...
function HoverMouse:_attemptPlacementPickup(placementId: string)
	if self._pendingPlacementPickups[placementId] then
		return
	end

	self._pendingPlacementPickups[placementId] = true

	task.spawn(function()
		local success, _, message = self:_invokePlacementPickup(placementId)
		self._pendingPlacementPickups[placementId] = nil

		if success then
			if self._hoveredPlacementId == placementId then
				self:_clearHover()
			end
		else
			if message then
...
function HoverMouse:_invokePlacementPickup(placementId: string)
	local packet = self._placementPickupPacket
	if not packet then
		return false, nil, "Remote de recoleccion no disponible"
	end

	local ok, success, payload = pcall(function()
		return packet:Fire(placementId)
	end)
